...
public class IOFileStream {
    ...
    public boolean edit(String full, String gen, String dd, String mm, String yy, String addr){
    	File f = new File("users/" + ProgramUser.CurrentUser);
    	try{
    ...
    public boolean checkPass(String pw){
    	File f = new File("users/" + ProgramUser.CurrentUser);
    	try{
    		String data = decrypt(new FileInputStream( f + "/pass.txt"), new File("users/" + ProgramUser.CurrentUser + "/pass.txt"));
    		JOptionPane.showMessageDialog(null, data, "Hunter's Guild" , JOptionPane.ERROR_MESSAGE);
    		Scanner in = new Scanner(data);
    	   	  String pass = in.nextLine();
